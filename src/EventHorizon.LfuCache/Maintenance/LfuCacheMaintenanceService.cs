...
{
    private static readonly TimeSpan _failureRetryDelay = TimeSpan.FromSeconds(1);
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            foreach (var cache in caches)
            {
                ...
                catch (Exception exception)
                {
                    ...
                    LfuCacheMaintenanceLog.StoreFailed(_logger, cache.Keyspace, exception);
                }
            }

            if (maintenanceFailed)
            {
                await Task.Delay(_failureRetryDelay, _timeProvider, stoppingToken).ConfigureAwait(false);
            }
        }
    }

...
- 键控服务（`keyed service`）注入，服务键（service key）即 keyspace；未指定时使用 `default`。
- 每个 keyspace 拥有一个独立的 `ConcurrentDictionary<TKey, CacheEntry<TValue>>`。
- 类型化接口，以及使用方法泛型的非泛型接口；两者都转发到同一个字典。
- 每个缓存项（entry）的过期时间（expiry）、读取时失效和后台增量清理。
- 按比例批量淘汰，以按时间归一后的频次为第一排序键、新 entry 保护为第二排序键、LRU 为第三排序键。
- 容量溢出阈值（overflow watermark）和同步淘汰反压。
- 基于时间的频次衰减，读取和淘汰时惰性归一，后台维护时按增量提前归一。
- 每个 keyspace 限制正在运行的工厂函数（factory）数量，并让同一 key 的调用共享同一次执行。
- 命名选项（`named options`）整体热更新。
- 单一后台维护循环、统计、指标和结构化日志。
- 单个 key 的 `GetOrAdd` / `GetOrAddAsync` 击穿保护。

不包含分布式一致性、持久化、按字节计量、绝对内存上限或业务接入逻辑。`Capacity` 是按 entry 数量计算的淘汰
水位；尚未完成的工厂函数（pending factory）和并发操作可能使物理缓存项数量暂时超过该水位。仓库中的通用 Web API
示例使用 `[FromKeyedServices("sample")]` 演示键控注入。

## 2. keyspace 与类型约束

keyspace 同时是配置边界、DI 边界和存储边界。每个 keyspace 只能注册一种
`(TKey, TValue)` 类型组合，并只创建一个对应的 `ConcurrentDictionary<TKey, CacheEntry<TValue>>`。
...
在同一 keyspace 重复注册相同类型组合时，操作是幂等的；注册不同类型组合时立即抛出
...
- key 和 value 始终保留其实际泛型类型，不使用 `object` 底座，不引入值类型装箱。
...
对外仅提供两个缓存接口，以及选项（options）、统计信息（stats）和注册扩展方法。存储、维护、注册索引、校验和指标实现均为
`internal`。
...
`DynamicLfuCache` 不保存 entry。类型化方法先根据 keyspace 从注册表（registry）读取唯一的缓存句柄（cache handle），验证
...
`Clear()` 和 `GetStats()` 直接操作 keyspace 的唯一存储，不进行跨类型聚合。
...
每个 keyspace 对应一份命名的 `LfuCacheOptions`：
...
| `DefaultExpiry` | 未传 entry 的 expiry 时使用的相对过期时间；`null` 表示默认不过期 |
...
- `0 < EvictionRatio <= 0.5`
- `0 <= OverflowRatio <= 0.5`
- `DefaultExpiry` 未设置，或其值大于零
- `1s <= MaintenanceInterval <= 1h`
- `1s <= DecayInterval <= 24h`
- `MaxInflight` 未设置，或至少为 `1`

维护服务在宿主启动时主动解析每个缓存；缓存构造阶段校验完整的命名选项，非法配置使宿主启动失败。
运行期非法配置不会替换当前快照，并记录 Warning 日志。

### 4.2 整体热更新

内部将 options 转换为不可变的 `OptionsSnapshot`，并提前计算目标阈值（target）、硬阈值（hard limit）、扫描预算和时间戳间隔。
运行时通过一个 volatile 引用整体替换该快照；每次操作只读取一次快照。
...
选项发生变化时，频次状态会保留已经经过的逻辑轮次。若 `DecayInterval` 变化，下一次衰减周期从变更时刻开始；
...
需要通过配置提供程序绑定配置时，仍可使用 `IConfiguration` 重载，但常规注册方式是直接配置。

每次注册都会执行以下步骤：

1. 幂等地注册 catalog、registry、metrics、options validator、`TimeProvider.System` 和维护服务。
2. 校验该 keyspace 尚未绑定其他类型组合，也未被精确 keyed service 占用。
3. 以归一化 keyspace 作为 options name 注册配置，并启用启动时验证。
4. 注册闭合类型的 keyed singleton 内部 store，以及一个不实现 `IDisposable` 的
   `ILfuCache<TKey, TValue>` 转发 facade。
5. 为 keyspace 幂等注册一个 keyed singleton `ILfuCache`。
6. keyspace 为 `default` 时额外注册普通 singleton；普通服务转发到 keyed 服务，二者指向同一实例。

不注册 keyed open generic。

keyed singleton 默认延迟创建。`LfuCacheMaintenanceService.StartAsync` 遍历 catalog 并主动解析所有类型化缓存，
确保 registry 在宿主启动完成前包含每个 keyspace 的唯一存储，配置错误也在启动阶段暴露。

如果已有 keyed service 占用了归一化后的 keyspace，注册会直接抛出异常，避免 catalog 元数据与实际解析到的实现不一致。
catalog 和维护服务解析的是内部存储，因此之后注册的公开类型化服务覆盖注册不会阻止 keyspace 初始化。
...
`FrequencyState` 的低 32 位和高 32 位分别保存饱和频次计数，以及该 entry 最近观察到的逻辑衰减轮次。命中和惰性归一
通过对这个打包字执行 CAS 循环完成。计数从 1 开始，最大为 `uint.MaxValue`，每个已经经过的逻辑轮次衰减一次，
下限为 1。逻辑轮次根据 `TimeProvider` 提供的经过时间计算，不只依赖维护线程推进。`LastAccessTicks` 用于确定相同
频次和保护状态下的 LRU 顺序，`CreatedTicks` 决定 1 秒保护状态，`long.MaxValue` 表示永不过期。

击穿保护状态也保存在同一个主字典中，不创建第二个 entry 字典。`TryGet` 不把未完成 factory 当作命中；
并发 `GetOrAdd` 调用则共享该 entry 内的单次执行状态。统计信息查询会汇总条带计数器，因此统计快照不是
线性一致的瞬时观察。

### 6.2 引用比对删除

过期、淘汰和 factory 失败路径必须比较 entry 的引用：
...
内部使用 `GetTimestamp()`，并按 `TimestampFrequency` 换算持续时间，不能把单调时间戳当作
...
1. `TryGetValue`；不存在或 entry 尚未完成时，记录 miss。
2. 若 `now >= ExpiresAtTicks`，引用比对删除，成功时记录 expiration，并按 miss 返回。
3. 按已经经过的衰减轮次惰性归一 `FrequencyState`，再以原子方式增加饱和频次并更新 `LastAccessTicks`。
...
1. 优先使用显式 expiry，否则读取当前快照的 `DefaultExpiry`；参数为 `null` 时使用默认值，显式 `TimeSpan.Zero`
...
同一个 key 的并发调用只执行一次 factory。发现已有正在运行的 factory 时，调用会等待该 factory 的结果，不再占用新的名额。新 key
会先申请该 keyspace 的 `MaxInflight` 名额；没有可用名额时抛出 `InvalidOperationException`。`null` 表示限制
继承 keyspace 的 `Capacity`，正数可以覆盖该值。命中和普通 `Set` 不占用名额。

factory 成功后将原 entry 原子地发布为已完成状态；失败或取消时通过引用比对移除 entry，使后续调用可以重试。即使
...
每个异步调用者的 `cancellation token` 只会取消自身的等待。工厂函数接收共享 `token`，只有所有等待者都取消后
才会取消该 `token`。缓存命中时返回同步完成的 `ValueTask<TValue>`。

普通 `Set` 可以替换正在执行的 factory 对应的 entry。此时 factory 的调用者仍收到自己的结果，但 factory 完成后不能覆盖
`Set` 写入的新 entry。

### 7.4 `Remove` 与 `Clear`

`Remove` 删除调用时观察到的 entry 并减少 count。`Clear` 原子替换内部存储状态，避免
`dictionary.Clear()` 与并发新增造成 count 漂移；已经获取旧状态的操作不会影响新的字典。
...
1. 只枚举一次唯一的字典。在同一轮枚举中通过引用比对删除过期 entry，惰性归一观察到的频次，并将符合条件的
   entry 放入每个 keyspace 复用的候选队列（candidate queue）。
2. 根据当前 count 计算降到 target 还需删除的数量 `k`。
3. 使用候选队列维护容量为 `k` 的大顶堆，按最小的 `(Frequency, Protected, LastAccessTicks)` 选择候选，避免
   全量排序。频次优先；相同频次时，已结束 1 秒保护窗口的 entry 优先于仍受保护的 entry；再以 LRU 排序。
4. 引用比对删除候选，更新淘汰和批次统计。`finally` 中清空队列，避免批次之间保留 key 或 entry 引用。

新 entry 的保护仅在频次相同时作为次级规则；低频的新 entry 不能仅凭保护状态压过高频旧 entry。候选不足
时会纳入受保护 entry，保证淘汰可以推进。

仍在执行工厂函数的待完成 entry（pending entry）永远不作为淘汰候选。如果容量压力全部来自待完成 entry，淘汰会安排稍后重试；
每个工厂函数完成时也会重新检查水位。
...
| 淘汰批次 | `O(N log k)`，包含一次弱一致枚举和有界候选队列 |
| 维护扫描 | `O(scan budget)` |

## 10. 可观测性

组件通过名为 `EventHorizon.LfuCache` 的 `System.Diagnostics.Metrics` Meter 发布指标。OpenTelemetry SDK 宿主可以通过
...
指标带有 `keyspace` 和 `value_type` 标签（tag）。

hit 和 miss 的总数使用内部条带计数器。`GetStats()` 汇总这些条带，避免读取热路径集中写入同一个原子字段；
因此统计快照相对于并发操作而言是近似值。
...
- 过期回收与候选选择共用一次字典枚举，批次结束后每个 keyspace 的候选队列不保留引用。
- 配置整体热更新生效；非法配置保留旧快照。
- 引用比对删除不会删除后来写入的新值。
- 并发 `GetOrAdd` 工厂函数只执行一次，失败后可以重试。
- `MaxInflight` 会拒绝超额的新 key 工厂函数；同一 key 的调用共享一次执行，运行中的工厂函数直到退出才释放名额。

DI 与动态入口：

- `default` 的 keyed 与普通解析返回同一个类型化缓存和同一个动态缓存。
- keyspace 大小写与首尾空白归一化。
- 同一 keyspace 重复注册相同类型组合幂等，注册不同组合立即失败。
- 动态入口写入的值可从类型化接口读取，证明两者共享唯一字典。
- 动态调用使用错误类型时抛出明确异常。
- 宿主启动后，尚未被业务解析的缓存也已进入 registry。

并发验收：

- 混合 get、set、remove 和维护操作不会出现死锁或未处理异常。
- 操作结束并完成维护后，内部 count 与唯一字典的实际 entry 数一致。
- count 越过 hard limit 后能通过同步或后台淘汰回到目标水位。

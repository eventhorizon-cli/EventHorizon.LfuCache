...
public sealed class OptionsValidationTests
{
    ...
    [Fact]
    public void AddLfuCache_InvalidOptions_AreRejectedByOptionsPipeline()
    {
        var services = new ServiceCollection();
        services.AddLfuCache<int, string>("invalid", options => options.Capacity = 0);
        using var provider = services.BuildServiceProvider();
        var monitor = provider.GetRequiredService<IOptionsMonitor<LfuCacheOptions>>();

        var exception = Assert.Throws<OptionsValidationException>(() => monitor.Get("invalid"));

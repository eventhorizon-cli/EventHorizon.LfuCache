...
public sealed class CacheBehaviorTests
{
    ...
    public async Task DynamicCache_ForwardsMutationsAndReads_ToRegisteredTypedStore()
    {
        ...
        var added = await dynamic.GetOrAddAsync<int, string>(2, static (_, _) =>
            new ValueTask<string>("two"), cancellationToken: TestContext.Current.CancellationToken);
    ...
    [Fact]
    public async Task GetOrAddAsync_ExistingValue_CompletesSynchronouslyWithoutCallingFactory()
    {
        using var host = new TestCacheHost<int, string>();
        host.Cache.Set(1, "cached");
        var factoryCalled = false;

        var pending = host.Cache.GetOrAddAsync(
            1,
            (_, _) =>
            {
                factoryCalled = true;
                return new ValueTask<string>("replacement");
            },
            cancellationToken: TestContext.Current.CancellationToken);

        Assert.True(pending.IsCompletedSuccessfully);
        Assert.Equal("cached", await pending);
        Assert.False(factoryCalled);
    }
